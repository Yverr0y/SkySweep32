...
- **Host Unit Tests for Parsers**: Added `test/host/` — desktop-compiled tests (system `g++`, no ESP32 toolchain/PlatformIO registry) that build the CRSF/MAVLink parsers against a minimal `Arduino.h` stub with AddressSanitizer/UBSan. They lock in the buffer-safety fixes (hostile length bytes rejected, valid frames round-trip) and run in CI via a new `host-tests.yml` workflow.
