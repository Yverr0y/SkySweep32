...
def render(name, objects, *, top=False):
    ...
    axis.view_init(elev=90 if top else 27, azim=-90 if top else -55)
    axis.set_axis_off()
...
def main():
    ...
    oled = cad.placed(Part.read(str(cad.OLED_STEP)), 60.0, -55.0, 9.3)
    ...
    render(
        "closed",
        [(base, blue, 1.0, 0.6), (pcba, green, 1.0, 0.8), (oled, black, 1.0, 0.5),
         (lid, light, 0.32, 0.6)] + [(shape, cap, 1.0, 0.35) for shape in buttons]
    ...
    lifted_lid = cad.placed(lid, 0.0, 0.0, 45.0)
    lifted_oled = cad.placed(oled, 0.0, 0.0, 45.0)
    lifted_buttons = [cad.placed(shape, 0.0, 0.0, 45.0) for shape in buttons]
    render(
        "open",
        [(base, blue, 0.10, 0.6), (pcba, green, 1.0, 0.8),
    ...
    render(
        "exploded",
        [(base, blue, 0.85, 0.6), (exploded_pcba, green, 1.0, 0.8),
    ...
    cut = cad.box(140.0, 50.0, 100.0, -10.0, -40.0, -10.0)
    render(
        "cutaway",
        [(base.cut(cut), blue, 1.0, 0.6), (pcba.cut(cut), green, 1.0, 0.8),
         (oled.cut(cut), black, 1.0, 0.5), (lid.cut(cut), light, 0.50, 0.6)],
    )
